...
static bool cmdlineCheckBinaryType(honggfuzz_t* hfuzz) {
    ...
    if (getenv("HFUZZ_DISABLE_PERSISTENT_AUTODETECT")) {
        LOG_I("Persistent auto-detection disabled by HFUZZ_DISABLE_PERSISTENT_AUTODETECT");
    } else if (memmem(map, fileSz, _HF_PERSISTENT_SIG, strlen(_HF_PERSISTENT_SIG))) {
...
bool cmdlineParse(int argc, char* argv[], honggfuzz_t* hfuzz) {
    *hfuzz = (honggfuzz_t){
    ...
    struct custom_option custom_opts[] = {
        ...
        { { "custom-mutator", required_argument, NULL, 0x117 }, "Unix socket for an in-process custom mutator" },
    ...
    for (;;) {
        ...
        case 0x110:
            ...
            break;
        case 0x117:
            hfuzz->exe.customMutatorSocket = optarg;
            break;
        case 0x106:
