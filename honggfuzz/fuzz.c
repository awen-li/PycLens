...
#define PYMORPH_RPC_MAGIC 0x504d5246U
#define PYMORPH_RPC_VERSION 1U
#define PYMORPH_RPC_STATUS_OK 0U
...
static bool fuzz_writeFull(int fd, const void* buf, size_t size) {
...
static bool fuzz_readFull(int fd, void* buf, size_t size) {
...
static bool fuzz_loadFileIntoRun(run_t* run, const char* path) {
    int fd = open(path, O_RDONLY | O_CLOEXEC);
    if (fd == -1) {
        PLOG_W("Custom mutator returned unreadable path '%s'", path);
        return false;
    }
    defer {
        close(fd);
    ...
    if (sz <= 0) {
        LOG_W("Custom mutator returned empty or unreadable variant '%s'", path);
        return false;
    }
    input_setSize(run, (size_t)sz);
    snprintf(run->dynfile->path, sizeof(run->dynfile->path), "%s", path);
    run->dynfile->imported = false;
...
static bool fuzz_customMutate(run_t* run) {
    const char* sock_path = run->global->exe.customMutatorSocket;
    if (sock_path == NULL || sock_path[0] == '\0') {
        return false;
    }
    if (strlen(sock_path) >= sizeof(((struct sockaddr_un*)0)->sun_path)) {
        LOG_W("Custom mutator socket path is too long: '%s'", sock_path);
        return false;
    }

    int fd = socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0);
    if (fd == -1) {
        PLOG_W("Couldn't create custom mutator socket");
        return false;
    }
    defer {
        close(fd);
    ...
    if (connect(fd, (const struct sockaddr*)&addr, sizeof(addr)) == -1) {
        PLOG_W("Couldn't connect to custom mutator socket '%s'", sock_path);
        return false;
    }

    size_t path_len = strnlen(run->dynfile->path, sizeof(run->dynfile->path));
    if (path_len > UINT32_MAX || run->dynfile->size > UINT32_MAX ||
        run->global->mutate.maxInputSz > UINT32_MAX) {
        LOG_W("Custom mutator request is too large");
        return false;
    }

    custom_mutator_request_t req = {
        .magic     = PYMORPH_RPC_MAGIC,
        .version   = PYMORPH_RPC_VERSION,
        .iteration = ATOMIC_GET(run->global->cnts.mutationsCnt),
        .fuzz_no   = run->fuzzNo,
        .max_size  = (uint32_t)run->global->mutate.maxInputSz,
        .path_len  = (uint32_t)path_len,
        .data_len  = (uint32_t)run->dynfile->size,
    };
    if (!fuzz_writeFull(fd, &req, sizeof(req)) ||
        !fuzz_writeFull(fd, run->dynfile->path, path_len) ||
        !fuzz_writeFull(fd, run->dynfile->data, run->dynfile->size)) {
        LOG_W("Couldn't send custom mutator request");
        return false;
    }

    custom_mutator_response_t resp;
    if (!fuzz_readFull(fd, &resp, sizeof(resp))) {
        LOG_W("Couldn't read custom mutator response");
        return false;
    }
    if (resp.magic != PYMORPH_RPC_MAGIC || resp.version != PYMORPH_RPC_VERSION ||
        resp.status != PYMORPH_RPC_STATUS_OK || resp.path_len == 0 || resp.path_len >= PATH_MAX) {
        LOG_D("Custom mutator declined request status=%u path_len=%u", resp.status, resp.path_len);
        return false;
    }

    char variant_path[PATH_MAX];
    if (!fuzz_readFull(fd, variant_path, resp.path_len)) {
        LOG_W("Couldn't read custom mutator variant path");
        return false;
    }
    variant_path[resp.path_len] = '\0';
    return fuzz_loadFileIntoRun(run, variant_path);
}
...
static bool fuzz_fetchInput(run_t* run) {
    {
        ...
        if (st == _HF_STATE_DYNAMIC_DRY_RUN) {
            ...
            if (input_prepareStaticFile(run, /* rewind= */ false, /* mangle= */ false)) {
                if (run->global->exe.customMutatorSocket) {
                    fuzz_customMutate(run);
                }
                return true;
            }
    ...
    if (fuzz_getState(run->global) == _HF_STATE_DYNAMIC_MAIN) {
        ...
        } else if (run->global->exe.customMutatorSocket) {
            if (!input_prepareDynamicInput(run, false)) {
                LOG_E("input_prepareDynamicInput() failed");
                return false;
            }
