...
`PycLens` is a Python package for the Python bytecode security empirical study. It supports the study workflow components:
...
```bash
pyclens collect --items-per-source 1000
```
...
```bash
pyclens collect --package-file data/packages.txt
```
...
```bash
pyclens prepare-analysis-env
pyclens analyze-tools
```
...
data/rq3/<cpython-version>/unique_bug_report.md
...
each version, PycLens downloads or reuses the matching CPython
...
`fuzz-cpython` uses the honggfuzz tree selected by `PYCLENS_HONGGFUZZ_HOME`.
...
tool-bounded: if no source candidate reproduces a finding, PycLens reports it
