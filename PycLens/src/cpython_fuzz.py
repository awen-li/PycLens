...
def download_cpython_archive(url: str, archive: Path, timeout: int) -> None:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "PycLens-study-tool/0.1"})
...
def run_seed_generator(interpreter: str, out_dir: Path, timeout: int) -> tuple[str, str]:
    script = textwrap.dedent(
        ...
        for name, source in snippets.items():
            try:
                code = compile(source, f"<pyclens-rq3-{name}>", "exec")
            except SyntaxError:
                continue
            write_pyc(name, code)
            written += 1
...
def write_harness(path: Path) -> None:
    ...
    path.write_text(
        textwrap.dedent(
            ...
            code = None
            try:
                code = importlib.machinery.SourcelessFileLoader("pyclens_rq3_seed", path).get_code("pyclens_rq3_seed")
            ...
            if code is not None:
                ...
                marshal.dumps(code)
                namespace = {"__name__": "pyclens_rq3_seed"}
                exec(code, namespace, namespace)
                target = namespace.get("__pyclens_seed__")
                if callable(target):
                    result = target()
...
def pyclens_env_path(name: str) -> Path | None:
...
def find_honggfuzz(explicit: Path | None = None) -> str | None:
    ...
    env_root = pyclens_env_path("PYCLENS_HONGGFUZZ_HOME")
    if env_root:
...
def prepare_instrumented_interpreters(
    ...
    for tag in sorted(set(tags)):
        ...
        if not base_interpreter:
            prepared[tag] = None
            print(
                f"[rq3-build] {tag}: missing base interpreter; run pyclens prepare-analysis-env first",
                flush=True,
            )
            continue
        status, reason, source_dir = prepare_cpython_source(
...
def validate_instrumented_cpython(python_bin: Path, timeout: int) -> bool:
    script = (
        ...
        "hashlib.md5(b'pyclens').hexdigest(); "
        "compile('x = 1\\n', '<pyclens-build-check>', 'exec')"
    )
    try:
        completed = subprocess.run(
