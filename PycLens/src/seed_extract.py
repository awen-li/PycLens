"""Extract PycLens fuzzing seeds from CPython unittest files."""
...
SEED_TARGET_NAME = "__pyclens_seed__"
...
def _render_case_seed(source: Path, case: CaseSeed) -> str:
    ...
    return (
        f"# pyclens-seed-target: {SEED_TARGET_NAME}\n"
        f"# source: {source}\n"
        f"# case: {case.name}\n\n"
        f"{body}\n"
    )
...
def _case_body(case: CaseSeed) -> list[ast.stmt]:
    prelude = "self = __pyclens_self__ = object()\n"
    if case.method:
        prelude += "__pyclens_self__ = self\n"
