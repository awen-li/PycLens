...
def rerun_debug_header(command: Sequence[str], cwd: str, env: dict[str, str]) -> str:
    lines = [
        "[pyclens-rerun] command=" + " ".join(command),
        "[pyclens-rerun] cwd=" + cwd,
        "[pyclens-rerun] PYTHONHOME=" + env.get("PYTHONHOME", ""),
        "[pyclens-rerun] PYTHONPATH=" + env.get("PYTHONPATH", ""),
        "[pyclens-rerun] PYTHONNOUSERSITE=" + env.get("PYTHONNOUSERSITE", ""),
    ]
    return "\n".join(lines) + "\n"
