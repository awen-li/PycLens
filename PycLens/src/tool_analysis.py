...
def global_tool_package(tool: str) -> str:
    package = first_env_value(GLOBAL_TOOL_PACKAGE_ENVS.get(tool))
    return package or GLOBAL_TOOL_PACKAGES.get(tool, tool)
...
def has_global_tool_install_command(tool: str) -> bool:
    return bool(first_env_value(GLOBAL_TOOL_INSTALL_CMD_ENVS.get(tool)))


def global_tool_install_command(env_dir: Path, tool: str, package: str) -> str:
    template = first_env_value(GLOBAL_TOOL_INSTALL_CMD_ENVS.get(tool))
    if not template:
        return ""
...
def analyze_pyc_entry(
    ...
    with tempfile.TemporaryDirectory(prefix="pyclens-rq2-") as tmpdir:
